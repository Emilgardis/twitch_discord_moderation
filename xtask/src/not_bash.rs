...
impl Drop for Pushd {
    fn drop(&mut self) { Env::with(|env| env.popd()) }
}
...
impl Drop for Pushenv {
    fn drop(&mut self) { Env::with(|env| env.popenv()) }
}
...
pub fn date_iso() -> Result<String> { run!("date --iso --utc") }
...
impl Env {
    ...
    fn cwd(&self) -> &Path { self.pushd_stack.last().unwrap() }
}
